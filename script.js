...
(function(){
  ...
  function initScreen5() {
    ...
    // Canvas 爱心粒子延迟 300ms 启动，让 BGM2 先完成解码
    setTimeout(function() {
      initScreen5Canvas();
    }, 300);
    // 生成彩带/爱心飘落
  ...
  function playBGM2() {
    ...
    if (p && p.then) {
      p.catch(function() {
        // 移动端首次播放可能因音频通道未激活失败，150ms后重试
        setTimeout(function() {
          bgm2.currentTime = 0;
          bgm2.play().catch(function(){});
        }, 180);
      });
    }
  }
